...
class ETSGD(Optimizer):
    def __init__(self, params, lr=1e-3, gam=0.99, lam=0.9):
            # check boundaries
            defaults={'lr': lr, 'gam': gam, 'lam': lam}
            super().__init__(params, defaults)
            for group in self.param_groups:
                for p in group['params']:
...
class ETAdam(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.99,0.999), eps=1e-8, gam=0.99, lam=0.9):
        # check boundaries
        defaults={'lr': lr, 'beta1': betas[0], 'beta2': betas[1], 'eps':eps, 'gam': gam, 'lam': lam}
        super().__init__(params, defaults)
        for group in self.param_groups:
            for p in group['params']:
                self.state[p] = dict(exp_avg=torch.zeros_like(p.data),
...
class RETAdam(Optimizer):
    def __init__(self, params, lr=1e-3, betas=(0.99,0.999), eps=1e-8, gam=0.99, lam=0.9, clip=0.2):
        # check boundaries
        defaults={'lr': lr, 'beta1': betas[0], 'beta2': betas[1], 'eps':eps,
                    'gam': gam, 'lam': lam, 'clip': clip}
        super().__init__(params, defaults)
        for group in self.param_groups:
            for p in group['params']:
                self.state[p] = dict(exp_avg=torch.zeros_like(p.data),
